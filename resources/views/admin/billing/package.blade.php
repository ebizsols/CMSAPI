...
@section('content')
    <div class="row">
        <div class="col-md-12">
            ...
            @endif
                <div class="white-box ">
                    <h1>@lang('app.monthly') @lang('app.menu.packages')</h1>
                    <div class="table-responsive table-responsive-froid">
                        <table class="table table-hover table-bordered text-center" >
                            ...
                            <tr>
                                ...
                                @foreach($packages as $package)
                                    <td class="@if(($package->id == $company->package->id && $company->package_type == 'monthly')) selected-plan @endif"><h3 class="panel-title price ">{{ $company->package->currency->currency_symbol }}{{ round($package->monthly_price) }}</h3></td>
                                @endforeach
                            </tr>
                                    ...
                                    <td>
                                        @if(round($package->monthly_price) > 0)
                                            @if(!($package->id == $company->package->id && $company->package_type == 'monthly')  && ($stripeSettings->paypal_status == 'active' || $stripeSettings->stripe_status == 'active' || $stripeSettings->razorpay_status == 'active' || $offlineMethods > 0))
                                                <button type="button" data-package-id="{{ $package->id }}" data-package-type="monthly" class="btn btn-success waves-effect waves-light selectPackage" title="Choose Plan"><i class="icon-anchor display-small"></i><span class="display-big">@lang('modules.billing.choosePlan')</span></button>
                                            @endif
                                        @endif
                                    </td>
                                @endforeach
                    ...
                    <div class="table-responsive table-responsive-froid">
                        <table class="table table-hover table-bordered text-center" >
                            ...
                            <tr>
                                <td><br>@lang('app.price')</td>
                                @foreach($packages as $package)
                                    <td class="@if(($package->id == $company->package->id && $company->package_type == 'annual')) selected-plan @endif"><h3 class="panel-title price">{{ $company->package->currency->currency_symbol }}{{ round($package->annual_price) }}</h3></td>
                                @endforeach
                            </tr>
                                ...
                                @foreach($packages as $package)
                                    <td>
                                        @if(round($package->annual_price) > 0)
                                            @if(!($package->id == $company->package->id && $company->package_type == 'annual') && ($stripeSettings->paypal_status == 'active'  || $stripeSettings->stripe_status == 'active'  || $stripeSettings->razorpay_status == 'active' || $offlineMethods > 0))
                                                <button type="button" data-package-id="{{ $package->id }}" data-package-type="annual" class="btn btn-success waves-effect waves-light selectPackage" title="Choose Plan"><i class="icon-anchor display-small"></i><span class="display-big">@lang('modules.billing.choosePlan')</span></button>
                                            @endif
                                        @endif
                                    </td>
                                @endforeach
