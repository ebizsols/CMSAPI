...
class AdminContractRenewController extends AdminBaseController
{
    ...
    public function renew(RenewRequest $request, $id)
    {
        ...
        $contractRenew->start_date = Carbon::createFromFormat('m/d/Y', $request->start_date_1)->format('Y-m-d');
        $contractRenew->end_date = Carbon::createFromFormat('m/d/Y', $request->end_date_1)->format('Y-m-d');
        $contractRenew->save();
